...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_student (col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""",
                        ('John', 'Doe', 'John Doe', '111-111-1111', 'jdoe@email.com', 'Software Development Boot Camp'))
            conn.commit()
    conn.close()
...
def onSelect(self, event):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute(
            """SELECT col_fname,col_lname,col_phone,col_email,col_course FROM tbl_student WHERE col_fullname = (?)""", [value])
...
def submit(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0):
        ...
        with conn:
            ...
            cursor.execute("""SELECT COUNT(col_fullname) FROM tbl_student WHERE col_fullname = '{}'""".format(
        ...
        messagebox.showerror(
            "Missing Text Error", "Please ensure that there is data in all five fields.")
        ...
        if count > 1:
            ...
            if confirm:
                ...
                with conn:
                    cursor = conn.cursor()
                    cursor.execute(
                        """DELETE FROM tbl_student WHERE col_fullname = '{}'""".format(var_select))
                onDeleted(
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0) and (len(var_course) > 0):
        ...
        with conn:
            ...
            cur.execute(
                """SELECT COUNT(col_phone) FROM tbl_student WHERE col_phone = '{}'""".format(var_phone))
            count = cur.fetchone()[0]
            print(count)
            cur.execute(
                """SELECT COUNT(col_email) FROM tbl_student WHERE col_email = '{}'""".format(var_email))
            count2 = cur.fetchone()[0]
            print(count2)
            cur.execute(
                """SELECT COUNT(col_course) FROM tbl_student WHERE col_course = '{}'""".format(var_course))
            ...
            if count == 0 or count2 == 0 or count3 == 0:
                ...
                if response:
                    ...
                    with conn:
                        cursor = conn.cursor()
                        cursor.execute("""UPDATE tbl_student SET col_phone = '{0}',col_email = '{1}' WHERE col_fullname = '{2}'""".format(
                ...
                tk.messagebox.showinfo("No changes detected", "The ({}), (()) and ({}) \nalready exist in the database for this name. \n\nYour update request has been cancelled.".format(
                    var_phone, var_email, var_course))
            onClear(self)
        conn.close()
    else:
